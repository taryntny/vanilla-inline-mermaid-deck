...
function initializeContent() {
  if (typeof DEFAULT_DECK_CONTENT !== "undefined") {
    ...
    console.log(
      "Content initialized:",
      Object.keys(currentContent).length,
      "slides"
    );
  } else {
...
async function renderContent() {
  ...
  if (!currentContent || Object.keys(currentContent).length === 0) {
    ...
    console.log(
      "DEFAULT_DECK_CONTENT available:",
      typeof DEFAULT_DECK_CONTENT !== "undefined"
    );
    if (typeof DEFAULT_DECK_CONTENT !== "undefined") {
      currentContent = JSON.parse(JSON.stringify(DEFAULT_DECK_CONTENT));
  ...
  if (missingCount > 0) {
    console.warn(
      `${missingCount} content elements not found. First 10:`,
      missingKeys.slice(0, 10)
    );
    // Log a sample of what's in currentContent
    if (missingCount > 5) {
      console.log(
        "Sample of currentContent keys:",
        Object.keys(currentContent).slice(0, 5)
      );
    }
  }
...
function openMermaidEditor(key) {
  ...
  document.getElementById("mermaidCode").value = getNestedValue(
    currentContent,
    activeDiagramKey
  );
...
window.onload = function () {
  ...
  if (
    ...
    console.error(
      "DEFAULT_DECK_CONTENT is not defined. Make sure content.js is loaded before script.js"
    );
...
function setupEventListeners() {
  ...
  if (clearCacheButton) {
    clearCacheButton.addEventListener("click", () => {
      // Clear localStorage
      localStorage.removeItem("mcp_deck_content");
      ...
      // Re-render with default content
      renderContent();
